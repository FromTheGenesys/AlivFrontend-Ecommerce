...
$( document ).ready( function() { 
    ...
    $( '.toprocessscreen' ).on( 'click', function() {

        var deviceindex   =   $(this).index( '.toprocessscreen');
        ...
        $( '.topaymentscreen' ).each( function( index, value ) {
            ...
            $( '.customerscreen').eq( index ).hide().fadeOut();
            $( '.paymentscreen').eq( index ).hide().fadeOut();
            $( '.processscreen').eq( index ).show().fadeIn();

            $.post( 'scripts/processTransaction.php',
            ...
            function( data ) { $( '.processedResult').eq( deviceindex ).html( data ); });
